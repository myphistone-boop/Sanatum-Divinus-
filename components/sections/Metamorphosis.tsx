      ...
      <section className="py-16 md:py-20 lg:py-24 px-6 relative z-10 overflow-hidden">
        <div className="max-w-[100rem] mx-auto">
            ...
            <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 items-stretch h-auto lg:h-[500px] xl:h-[650px]">
                ...
                <div className="lg:w-1/4 flex flex-col gap-2 lg:gap-3 justify-center">
                    {features.map((feature, idx) => (
                        <div 
                            ...
                            className={`
                                p-4 lg:p-5 xl:p-6 rounded-[1.5rem] lg:rounded-[2rem] cursor-pointer transition-all duration-500 border relative overflow-hidden group
                                ${activeFeature === idx 
                        ...
                        >
                            <div className="flex items-center gap-3 lg:gap-4 xl:gap-6 relative z-10">
                                ...
                                <div>
                                    ...
                                    <span className="text-[9px] lg:text-[10px] xl:text-xs uppercase tracking-widest opacity-60 mt-1 block font-sans font-semibold">{feature.subtitle}</span>
                                </div>
                            </div>
                        </div>
                    ))}
                ...
                {/* Right: Massive Content Card */}
                <div className="lg:w-3/4 relative">
                    <div 
                        key={activeFeature}
                        className="animate-fade-in-up w-full h-full bg-[#1A2B22]/90 backdrop-blur-2xl rounded-[2rem] lg:rounded-[2.5rem] xl:rounded-[3rem] border border-sacred-gold/20 p-6 lg:p-8 xl:p-12 relative overflow-hidden shadow-2xl group"
                    >
                        {/* Background Decor */}
                        ...
                        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 h-full">
                            
                            {/* Column 1: Text & Context */}
                            <div className="flex flex-col justify-between">
                                <div>
                                    <div className="flex items-center gap-3 lg:gap-4 mb-4 lg:mb-6">
                                        <div className="p-2 lg:p-2 bg-sacred-gold/10 rounded-full text-sacred-gold border border-sacred-gold/20">
                                            {features[activeFeature].icon}
                                        </div>
                                        <div className="text-sacred-gold uppercase tracking-[0.3em] text-[10px] lg:text-xs font-bold">Thérapie Sacrée</div>
                                    </div>

                                    <h3 className="font-serif text-3xl lg:text-4xl xl:text-5xl text-sacred-cream mb-4 lg:mb-6 leading-[1.1]">
                                        {features[activeFeature].title}
                                    </h3>
                                    
                                    <p className="font-sans text-sm lg:text-base xl:text-lg text-sacred-cream/80 leading-relaxed mb-6 lg:mb-8 font-light text-justify border-l-2 border-sacred-gold/30 pl-4 lg:pl-6">
                            ...
                            <div className="bg-sacred-green-dark rounded-[1.5rem] lg:rounded-[2rem] p-5 lg:p-6 xl:p-8 border border-white/5 relative overflow-hidden flex flex-col justify-center">
                                <div className="absolute inset-0 bg-gradient-to-br from-sacred-gold/5 to-transparent"></div>
                                
                                <h4 className="font-serif text-xl lg:text-2xl text-sacred-cream mb-6 lg:mb-8 relative z-10">Le Protocole</h4>
                                
                                <div className="space-y-4 lg:space-y-6 relative z-10">
                                    {features[activeFeature].steps.map((step, i) => (
                                        <div key={i} className="flex items-center gap-4 group/step">
                                            <div className="relative">
                                                <div className="w-8 h-8 lg:w-10 lg:h-10 rounded-full border border-sacred-gold/40 bg-sacred-green-dark flex items-center justify-center text-sacred-gold text-sm font-bold shadow-[0_0_15px_rgba(217,185,94,0.2)] group-hover/step:scale-110 transition-transform">
                                                    {i + 1}
                                                </div>
                                                {i !== features[activeFeature].steps.length - 1 && (
                                                    <div className="absolute top-8 lg:top-10 left-1/2 w-[1px] h-4 lg:h-6 bg-sacred-gold/20 -ml-[0.5px]"></div>
                                                )}
                                            </div>
                                            <div className="flex-1 p-2 lg:p-3 rounded-lg bg-white/5 border border-white/5 group-hover/step:border-sacred-gold/30 transition-colors">
                                                <span className="text-sm lg:text-base text-sacred-cream/90 font-light tracking-wide">{step}</span>
                                            </div>
                                        </div>
                                    ))}
                                </div>

                                <div className="mt-6 lg:mt-8 flex items-center gap-2 lg:gap-3 text-sacred-gold/60 text-xs lg:text-sm justify-center border-t border-white/5 pt-4">
                                    <Check size={14} />
