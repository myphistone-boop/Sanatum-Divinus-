      ...
      <section className="py-32 px-6 relative z-10 overflow-hidden">
        <div className="max-w-[100rem] mx-auto">
            <div className="text-center mb-20">
                <h2 className="font-serif text-6xl md:text-8xl text-sacred-cream mb-6">Les Clés de la <span className="text-sacred-gold italic">Métamorphose</span></h2>
                <div className="w-32 h-1 bg-sacred-gold/30 mx-auto rounded-full"></div>
            </div>
            
            <div className="flex flex-col lg:flex-row gap-8 items-stretch h-auto lg:h-[700px]">
                ...
                <div className="lg:w-1/4 flex flex-col gap-4 justify-center">
                    {features.map((feature, idx) => (
                        <div 
                            ...
                            className={`
                                p-8 rounded-[2rem] cursor-pointer transition-all duration-500 border relative overflow-hidden group
                                ${activeFeature === idx 
                        ...
                        >
                            <div className="flex items-center gap-6 relative z-10">
                                ...
                                <div>
                                    <h3 className="font-serif text-2xl font-bold leading-none">{feature.title}</h3>
                                    <span className="text-xs uppercase tracking-widest opacity-60 mt-2 block font-sans font-semibold">{feature.subtitle}</span>
                                </div>
                            </div>
                        </div>
                    ))}
                ...
                <div className="lg:w-3/4 relative">
                    <div 
                        key={activeFeature}
                        className="animate-fade-in-up w-full h-full bg-[#1A2B22]/90 backdrop-blur-2xl rounded-[3rem] border border-sacred-gold/20 p-12 lg:p-16 relative overflow-hidden shadow-2xl group"
                    >
                        {/* Background Decor */}
                            ...
                            <div className="flex flex-col justify-between">
                                <div>
                                    <div className="flex items-center gap-4 mb-8">
                                        ...
                                        <div className="text-sacred-gold uppercase tracking-[0.3em] text-sm font-bold">Thérapie Sacrée</div>
                                    </div>

                                    <h3 className="font-serif text-5xl md:text-6xl text-sacred-cream mb-8 leading-[1.1]">
                                        {features[activeFeature].title}
                                    </h3>
                                    
                                    <p className="font-sans text-xl text-sacred-cream/80 leading-relaxed mb-10 font-light text-justify border-l-2 border-sacred-gold/30 pl-6">
                                    ...
                                    <div className="grid grid-cols-2 gap-6 mb-8">
                                        <div className="bg-white/5 rounded-2xl p-4 border border-white/5">
                                            ...
                                            <span className="text-sacred-cream text-lg">{features[activeFeature].science}</span>
                                        </div>
                                        <div className="bg-white/5 rounded-2xl p-4 border border-white/5">
                                            <div className="flex items-center gap-2 text-sacred-gold mb-2">
                                                <Clock size={18} />
                                            ...
                                            <span className="text-sacred-cream text-lg">{features[activeFeature].duration}</span>
                                        </div>
                                    </div>
                                </div>
                                
                                <Button variant="outline" onClick={() => onNavigate(View.DISCOVER)} className="w-max">
                                    Explorer en détail
                                </Button>
                            </div>

                            {/* Column 2: Process & Visuals */}
                            <div className="bg-sacred-green-dark rounded-[2.5rem] p-10 border border-white/5 relative overflow-hidden flex flex-col justify-center">
                                <div className="absolute inset-0 bg-gradient-to-br from-sacred-gold/5 to-transparent"></div>
                                
                                <h4 className="font-serif text-3xl text-sacred-cream mb-10 relative z-10">Le Protocole</h4>
                                
                                <div className="space-y-8 relative z-10">
                                    {features[activeFeature].steps.map((step, i) => (
                                        <div key={i} className="flex items-center gap-6 group/step">
                                            <div className="relative">
                                                <div className="w-12 h-12 rounded-full border border-sacred-gold/40 bg-sacred-green-dark flex items-center justify-center text-sacred-gold font-bold shadow-[0_0_15px_rgba(217,185,94,0.2)] group-hover/step:scale-110 transition-transform">
                                                    {i + 1}
                                                </div>
                                                {i !== features[activeFeature].steps.length - 1 && (
                                                    <div className="absolute top-12 left-1/2 w-[1px] h-8 bg-sacred-gold/20 -ml-[0.5px]"></div>
                                                )}
                                            </div>
                                            <div className="flex-1 p-4 rounded-xl bg-white/5 border border-white/5 group-hover/step:border-sacred-gold/30 transition-colors">
                                                <span className="text-lg text-sacred-cream/90 font-light tracking-wide">{step}</span>
                                            </div>
                                        </div>
                                    ))}
                                </div>

                                <div className="mt-10 flex items-center gap-3 text-sacred-gold/60 text-sm justify-center border-t border-white/5 pt-6">
                                    <Check size={16} />
