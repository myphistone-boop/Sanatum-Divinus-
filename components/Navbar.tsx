    ...
    <>
      <nav 
        className={`fixed top-4 md:top-6 left-1/2 transform -translate-x-1/2 z-50 transition-all duration-700 ease-in-out
          ${scrolled 
            ? 'w-[95%] max-w-[1400px] py-3 lg:py-5' 
            : 'w-[95%] max-w-[1800px] py-4 lg:py-8'}
        `}
      >
        <div className={`
        ...
        `}>
          ...
          >
            <div className={`
                ...
                -ml-2
                ${scrolled 
                  ? 'w-12 h-12 lg:w-16 lg:h-16 -my-4' 
                  : 'w-16 h-16 lg:w-24 lg:h-24 -my-6 lg:-my-12'}
            `}>
              <img 
                src={LOGO_URL} 
                alt="SD" 
                className={`
                  ...
                  ${scrolled ? 'w-8 h-8 lg:w-10 lg:h-10' : 'w-10 h-10 lg:w-14 lg:h-14'}
                `} 
              />
            </div>
            
            <span className={`font-serif tracking-widest text-sacred-gold text-base lg:text-2xl font-bold hidden lg:block opacity-0 ${scrolled ? 'opacity-0 w-0' : 'opacity-100 w-auto'} transition-all duration-500 overflow-hidden whitespace-nowrap pl-4`}>
              SANATUM DIVINUS
            </span>
          </div>
          ...
          <div className="hidden lg:flex items-center gap-8 xl:gap-12">
            {NAV_ITEMS.map((item) => (
              <button
                ...
                className={`
                  uppercase tracking-[0.2em] text-xs xl:text-sm font-bold transition-all duration-500 relative py-4
